#!/bin/bash
# This script recursively finds files with special characters in their filenames
# and replaces those characters with underscores
...
sanitize_filename() {
    ...
    # Create the new filename by replacing special characters with underscores
    # Keep original replacements and add handling for left and right curly quotes
    local new_base=$(echo "$base" | tr "[];'{\"}()" "_________")
    # Now replace left curly quote (U+201C) and right curly quote (U+201D)
    new_base=$(echo "$new_base" | sed $'s/\xe2\x80\x9c/_/g; s/\xe2\x80\x9d/_/g')
...
process_directory() {
    ...
    echo "$files" | grep -v "^$" | while read file; do
        ...
        # Check if filename contains special characters using the same logic as sanitize_filename
        base=$(basename "$file")
        new_base=$(echo "$base" | tr "[];'{\"}()" "_________")
        new_base=$(echo "$new_base" | sed $'s/\xe2\x80\x9c/_/g; s/\xe2\x80\x9d/_/g')
        ...
        if [ "$base" != "$new_base" ]; then
            ...
            echo "    No special characters to replace"
        fi
...
echo "Checking for any remaining files with special characters in other directories..."
process_directory "."

echo "All done! Replaced special characters with underscores in all filenames."
